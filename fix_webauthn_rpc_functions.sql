-- Drop existing functions first, then recreate
DROP FUNCTION IF EXISTS public.webauthn_get_existing_credentials(UUID);
DROP FUNCTION IF EXISTS public.webauthn_store_challenge(UUID, TEXT, TEXT, TIMESTAMPTZ);
DROP FUNCTION IF EXISTS public.webauthn_get_challenge(UUID, TEXT);
DROP FUNCTION IF EXISTS public.webauthn_mark_challenge_used(UUID);
DROP FUNCTION IF EXISTS public.webauthn_store_credential(UUID, TEXT, BYTEA, BIGINT, TEXT, BOOLEAN, TEXT[], TEXT);
DROP FUNCTION IF EXISTS public.webauthn_get_credential(TEXT);
DROP FUNCTION IF EXISTS public.webauthn_update_counter(TEXT, BIGINT);

-- Function to get existing credentials for a user
CREATE FUNCTION public.webauthn_get_existing_credentials(p_user_id UUID)
...
CREATE FUNCTION public.webauthn_store_challenge(
  p_user_id UUID,
  p_challenge TEXT,
...
CREATE FUNCTION public.webauthn_get_challenge(
  p_user_id UUID,
  p_challenge_type TEXT
)
RETURNS TABLE(
...
CREATE FUNCTION public.webauthn_mark_challenge_used(p_challenge_id UUID)
RETURNS BOOLEAN AS $$
BEGIN
...
CREATE FUNCTION public.webauthn_store_credential(
  p_user_id UUID,
...
CREATE FUNCTION public.webauthn_get_credential(p_credential_id TEXT)
RETURNS TABLE(
  id UUID,
  user_id UUID,
  credential_id TEXT,
...
CREATE FUNCTION public.webauthn_update_counter(
  p_credential_id TEXT,
