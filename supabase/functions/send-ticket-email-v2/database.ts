...
import { Order, PaymentMethodInfo } from './types.ts';
...
export class DatabaseService {
  ...
  async getPaymentCredentials(organizationId: string): Promise<any> {
    const { data: credentialsArray } = await this.client
  ...
  async generateTickets(orderItems: any[]): Promise<any[]> {
    ...
    for (const item of orderItems) {
      ...
      for (let i = 0; i < item.quantity; i++) {
        ...
        const { data: ticketCode, error: ticketError } = await this.client
          .rpc("generate_ticket_code")
          .single();

        if (ticketError) {
          throw new EmailServiceError(
            `Failed to generate ticket code: ${ticketError.message}`,
        ...
        if (insertError) {
          throw new EmailServiceError(
            `Failed to insert ticket: ${insertError.message}`,
  ...
  async getTicketQrUrls(orderItemIds: string[]): Promise<Record<string, string>> {
    if (orderItemIds.length === 0) return {};

    try {
      const { data: dbTickets } = await this.client
        .from('tickets')
      ...
      const codeToQrUrl: Record<string, string> = {};
      ...
      logStep("Failed to generate QR codes", { error });
      return {};
    }
  }
  ...
  async updateOrderStatus(orderId: string, status: string): Promise<void> {
    ...
    logStep("Order status updated", { orderId, status });
  }
  ...
  async sendOrganizerNotification(orderId: string): Promise<void> {
    try {
      ...
      logStep("Organizer notification sent", { orderId });
    } catch (error) {
